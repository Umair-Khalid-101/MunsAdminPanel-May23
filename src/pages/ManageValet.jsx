...
export default function ManageValet() {
          ...
          <tbody>
            {data.map((valet) => (
              <tr className="border-b bg-gray-800" key={Math.random()}>
                ...
                <td className="px-6 py-4">{valet?.role}</td>
                <td className="px-6 py-4">{valet?.status}</td>
                <td className="px-6 py-4">
                  {valet?.status === "Active" && (
                    <button
                      className="font-medium text-blue-600 dark:text-blue-500 hover:underline"
                      onClick={() => handleDeactivate(valet?.id)}
                    >
                      Deactivate
                    </button>
                  )}
                  {valet?.status === "InActive" && (
                    <button
                      className="font-medium text-blue-600 dark:text-blue-500 hover:underline"
                      onClick={() => handleActivate(valet?.id)}
                    >
                      Activate
                    </button>
                  )}
  ...
  return (
    <>
      ...
      {isLoading && <Loader title={"Getting Valets!"} />}
